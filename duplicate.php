...
function rudr_duplicate_post_link( $actions, $post ) {
    ...
    // Add links for multiple duplications
    $actions['duplicate_3'] = '<a href="' . esc_url( add_query_arg('copies', 3, $url) ) . '" title="Create 3 duplicates">Duplicate 3x</a>';
    $actions['duplicate_5'] = '<a href="' . esc_url( add_query_arg('copies', 5, $url) ) . '" title="Create 5 duplicates">Duplicate 5x</a>';
    $actions['duplicate_10'] = '<a href="' . esc_url( add_query_arg('copies', 10, $url) ) . '" title="Create 10 duplicates">Duplicate 10x</a>';
...
function rudr_duplicate_post_as_draft(){
    ...
    // Get number of copies to create (default: 1)
    $copies = isset($_GET['copies']) ? absint($_GET['copies']) : 1;
    ...
    // Get count of existing duplicates to add numeric suffix
    $args = array(
        'post_type' => $post->post_type,
        'post_status' => 'any',
        'posts_per_page' => -1,
        'title_like' => $post->post_title
    );
    $existing_copies = new WP_Query($args);
    $copy_number = $existing_copies->post_count + 1;
    ...
    if( $post ) {
        ...
        for ($i = 0; $i < $copies; $i++) {
            ...
            $args = array(
                ...
                'post_status'    => 'publish', // Changed from 'draft' to 'publish'
                'post_title'     => $post->post_title . ' ' . ($copy_number + $i), // Added numeric suffix
                'post_type'      => $post->post_type,
        ...
        // Store the number of copies created in a transient for the admin notice
        set_transient('rudr_copies_created', $copies, 60);
...
function rudr_duplication_admin_notice() {
    ...
    if( isset( $_GET[ 'saved' ] ) && 'post_duplicate_created' === $_GET[ 'saved' ] ) {
        $copies = get_transient('rudr_copies_created');
        if (!$copies) {
            $copies = 1;
        }
        delete_transient('rudr_copies_created');
        
        $message = $copies > 1 ? 
            sprintf('%d post copies created.', $copies) : 
            'Post copy created.';
